...
class WilsonMazeEvalCallback(BaseCallback):
    def __init__(self, eval_config, trial, best_model_save_path=None, logs_path=None, 
                 eval_freq=10000, deterministic=True, max_number_of_steps=15, use_action_masks=False, verbose=0):
            ...
            if self.logs_path is not None:
                with open(os.path.join(self.logs_path, 'evaluations.json'), 'r+') as log_file:
                    ...
                    old_data['evaluations'].append({'step': self.num_timesteps,
                                                    ...
                                                    'stats': stats,
                                                    'kwargs': json.dumps(self.eval_config)})
                    log_file.seek(0)
...
class Study:
    def __init__(self, n_timesteps=50000, n_evaluations=4, n_envs=8, verbose=0, logs_path='./logs', 
    ...
    def sample_ppo_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        ent_coef = trial.suggest_categorical("ent_coef", [0.001, 0.01, 0.1, 1, 5, 10, 15, 20, 25, 30])
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        vec_env = DummyVecEnv(
        [make_env(rank=i, x=self.X_train, y=self.y_train, seed=0, **self.env_train_configg)
        ...
        if self.use_action_masks:
            model = MaskablePPO("MlpPolicy", vec_env, device='auto', tensorboard_log=None, 
                                seed=None, verbose=self.verbose, **sampled_hyperparams)
        else:
...
if __name__ == '__main__':
    ...
    study = Study(n_timesteps=1e6, n_evaluations=4, n_envs=8, verbose=1, logs_path='./logs', use_action_maks=True)
    study.set_data(PROMPTS_FILE, TARGETS_FILE, limit=6000, test_size=0.2, seed=RANDOM_STATE)

    study.optimize(n_trials=10, timeout=3600, n_startup_trials=5, seed=seed + RANDOM_STATE)
